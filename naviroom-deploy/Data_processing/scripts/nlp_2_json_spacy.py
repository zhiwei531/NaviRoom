...
ROOM_TYPE_RULES: list[tuple[str, list[str]]] = [
    ...
    ("classroom",       ["classroom"]),
    ("office",          ["office"]),
    ("team room",       ["team room", "team space", "collaboration room"]),
    ("multi-media booth", ["multi-media booth", "multimedia booth", "media booth", "booth"]),
]
...
def parse_structured_row(row: dict) -> dict:
    ...
    return {
        ...
        "raw_description": ", ".join(f"{k}={v}" for k, v in row.items()
